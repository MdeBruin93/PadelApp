...
param azuresql_outputs_name string

param azuresql_outputs_sqlserveradminname string
...
resource azuresql 'Microsoft.Sql/servers@2023-08-01' existing = {
  name: azuresql_outputs_name
}

resource sqlServerAdmin 'Microsoft.ManagedIdentity/userAssignedIdentities@2024-11-30' existing = {
  name: azuresql_outputs_sqlserveradminname
}
...
resource script_azuresql_database 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  name: take('script-${uniqueString('azuresql', principalName, 'database', resourceGroup().id)}', 24)
  location: location
  identity: {
    type: 'UserAssigned'
  ...
  properties: {
    ...
    environmentVariables: [
      ...
      {
        name: 'DBSERVER'
        value: azuresql.properties.fullyQualifiedDomainName
      }
      {
        name: 'PRINCIPALTYPE'
        value: principalType
